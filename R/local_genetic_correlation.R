...
for (i in 1:nbin) {
    ...
    if (!is.null(locus)) {
        ...
        univ2 <- data.frame(h2.obs.phen1 = univ$h2.obs[1], h2.p.phen1 = univ$p[1], h2.obs.phen2 = univ$h2.obs[2], h2.p.phen2 = univ$p[2])
        bivar2 <- bind_cols(bivar, univ2)

        results <- bind_rows(results, cbind(loc.info, bivar2))
    }
}
