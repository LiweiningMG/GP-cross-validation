...
for (r in 1:opt$rep) { # nolint
  for (f in 1:opt$fold) {
    ...
    ## 匹配验证群ebv
    y_adj_ebv <- inner_join(y_adj, ebv_val, by = "id", sort = FALSE)
    ...
    ## 准确性(pearson相关系数)
    accur_rf <- cor(y_adj_ebv$yhat[ids_index], y_adj_ebv$ebv_val[ids_index])
    bias_rf <- lm(yhat ~ ebv_val, data = y_adj_ebv[ids_index, ])$coefficients[2]

    ## 保存结果
    accs$cor[accs$rep == r & accs$fold == f] <- round(accur_rf, opt$digit)
...
# CFJY within
opt <- list()
opt$tbvf="/BIGDATA2/cau_jfliu_2/liwn/mbGS/ChipSim/Lin2022/rep1/identical/cor0.6/SNT/SNT_dmu_pheno.txt"
opt$tbv_col=5
opt$val_idf = "/BIGDATA2/cau_jfliu_2/liwn/mbGS/ChipSim/Lin2022/rep1/identical/cor0.6/SNT/val#val#/rep#rep#/val.id"
opt$dir_val = "/BIGDATA2/cau_jfliu_2/liwn/mbGS/ChipSim/Lin2022/rep1/identical/cor0.6/union/val#val#/rep#rep#/union"
opt$fold=5
opt$rep=5
opt$add_sol=3
opt$out = "/BIGDATA2/cau_jfliu_2/liwn/mbGS/ChipSim/Lin2022/rep1/identical/cor0.6/union/accur_GBLUP_SNT.txt"
