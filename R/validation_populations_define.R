...
if (!is.null(opt$rminvail)) {
    ...
    if (any(ind_remove)) {
        ...
        if (!is.null(opt$fam) && !is.null(opt$keepid)) {
            ...
            if (any(!keep_index)) {
                write.table(subset(fam, keep_index, select= 1:2), opt$keepid, row.names = FALSE, col.names = FALSE, quote =     FALSE)
...
## debug
opt <- list()
opt$phef <- "/BIGDATA2/cau_jfliu_2/liwn/mbGS/Real/rmodel/ADG/DD/pheno_dmu_DD.txt"
opt$nonmiss <- "4 2 1"
opt$rep <- 5
opt$fold <- 5
opt$outvid <- "val.id"
opt$outdir <- "/BIGDATA2/cau_jfliu_2/liwn/mbGS/Real/rmodel/ADG/DD/val#val#/rep#rep#"
opt$rminvail <- TRUE
opt$keepid <- "/BIGDATA2/cau_jfliu_2/liwn/mbGS/Real/rmodel/ADG/DD/keep_fid_id.txt"
opt$pheCol <- "5"
opt$fam <- "/BIGDATA2/cau_jfliu_2/liwn/mbGS/Real/rmodel/ADG/DD/DD.fam"
